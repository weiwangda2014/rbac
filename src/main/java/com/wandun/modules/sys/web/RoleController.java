...
public class RoleController extends BaseController {
    ...
    public Message save(Role role, BindingResult bindingResult) {
        if (!UserUtils.getUser().isAdmin() && role.getSysData().equals(Global.YES)) {
            return Message.error("越权操作，只有超级管理员才能修改此数据！");
        }
        if (Global.isDemoMode()) {
            Message.error("演示模式，不允许操作！");
        }
        Message errorMessage = BeanValidators(role, bindingResult);
        if (errorMessage != null) {
            return errorMessage;
        }
        if (!"true".equals(checkName(role.getOldName(), role.getName()))) {
            return Message.error("保存角色'" + role.getName() + "'失败, 角色名已存在");
        }
        if (!"true".equals(checkEnname(role.getOldEnname(), role.getEnname()))) {
            return Message.error("保存角色'" + role.getName() + "'失败, 英文名已存在");
        }
